...
Conversion and sum/mean/minimum/maximum use exact rational arithmetic; only the final daily result is rounded. The aggregate binds hashes of the governing contract, recomputed coverage summary, complete conversion-rule set, policy and sorted numeric source set. Its values and full result are not yet part of the report projection or receipt manifest and do not establish that a rule or policy is approved outside this synthetic boundary.

Iteration 8 pins the aggregate-policy and aggregate-evaluation hashes into receipt provenance, but the aggregate values and full aggregate result are still absent from `ReportContentProjection` and all rendered artifacts. The pin proves which internal evaluation accompanied the coverage report; it does not make the numeric result a regulatory report field or establish that a rule or policy was approved outside this synthetic boundary.

### Reconciled evidence evaluation contract

`evaluateReconciledCsvEvidence` is the strict synthetic composition boundary from exact CSV bytes through the existing unsigned receipt and version-1 frozen draft. The input has an independent set of one to 64 `RequiredSeriesContract`s. It must also have exactly one series bundle for every contract ID/version and no extra bundle; the contracts share tenant/system scope and remain authoritative even when no source exists. Each bundle carries its CSV source contract, measurement mapping, conversion rules, daily aggregate policy, an optional approved `plausibility-policy/v1` (ADR-0014) and zero to 64 exact `Uint8Array` source objects. The complete evaluation is capped at 64 submitted source objects and 64 MiB of source bytes, so multiple bundles share both global budgets. Intrinsic typed-array slots establish byte lengths and snapshots; shared or resizable backing storage is rejected.

Before inspecting source availability, the evaluator reconstructs and content-addresses the required-series contract, CSV contract, mapping, conversion-rule set and plausibility policy as `csv-measurement-governance-binding/v2`, whose `plausibilityPolicyHash` is `null` where no policy is approved rather than absent. Governance is therefore source-independent and cannot be supplied as caller-created hashes. A non-empty bundle reruns ADR-0007 and returns a tagged `reconciled` state. An empty bundle returns `no_source_objects`, the same validated governance, a null reconciliation result and a deterministic operational hash. It then sends the reconciliation-derived observations—or an empty observation set—to coverage, so an expected source-less series remains explicit gaps rather than disappearing from the required set.

Each series has two intentionally different identities:

- `evidenceSetHash` binds governance, unique source summaries without submission multiplicity, reconciliation outcomes, final observations and numeric preimages. Repeating an exact source delivery does not change this hash.
- `operationalHash` is the reconciliation hash when sources exist and a typed governance hash when none exist. Submitted-source multiplicity remains part of the reconciliation result, so an exact retry changes this operational hash. The root `evaluationHash` binds the complete evaluation result and therefore changes with that operational accounting.

Reconciliation observations alone feed coverage. The evaluator selects numeric preimages by the observation IDs that coverage classified as accepted winners, reduces the conversion-rule set to those referenced winners and runs exact daily aggregation. Duplicate, quarantined, conflicting, scheduled-nonoperation and lifecycle-ineligible observations cannot contribute a value. When coverage has no numeric winner, the aggregate is still deterministically evaluated with empty `values`; the evaluator never manufactures a zero or selects a fallback.

All receipt provenance is derived: globally deduplicated exact source-object hashes; required-series, CSV-contract, mapping, conversion-rule-set and aggregate-policy hashes; the retry-insensitive evidence-set hash; and the daily aggregate-evaluation hash. Those values are pinned through the existing unsigned `EvidenceManifest`, which produces the existing deterministic render/receipt and then the version-1 frozen draft. Operational delivery multiplicity is deliberately not pinned into the receipt, so an exact retry preserves receipt and frozen-draft IDs while the root evaluation hash records the different operation.

`validateReconciledCsvEvidenceIntegrity` is the matching retained-result boundary. It reruns the complete evaluator from exact authoritative inputs, then recursively reconstructs the supplied result against that replay's exact branch and keys. Plain or null-prototype records with own enumerable data properties and dense ordinary mutable/frozen arrays are accepted; symbols, extras, omissions, accessors, custom prototypes, sparse arrays and every primitive divergence fail closed. It compares reconciliation accounting, governance and evidence identities, coverage, numeric aggregates, readiness, receipt, frozen draft and root hash. The validator returns the newly replayed deeply frozen result, never the caller's object. It adds no schema or identity and does not turn deterministic integrity into authenticity.

This boundary is synthetic and in memory. CSV timestamps remain fixed-millisecond UTC; there is no real reviewed vendor format, immutable source/normalized storage, database or worker concurrency, durable idempotency, authenticated correction/supersession, or numeric report projection. It narrows but does not close BL-033, BL-038, BL-040, BL-042, BL-043, BL-047, BL-055 or BL-056.
