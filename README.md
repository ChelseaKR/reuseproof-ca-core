...
| Release & Versioning | Applies | Applies — SemVer, `CHANGELOG.md` with `[Unreleased]`, dispatch-triggered `release.yml` that re-verifies at the tagged commit. Version `0.1.0` is declared in `package.json` and `CITATION.cff` and no tag names it, so nothing has been released: no `v0.1.0` tag, no GitHub Release, no package-registry publication. `tests/release-claims.test.ts` derives that from `git tag --list` rather than restating it, and binds the citation release date and the dated changelog section to the tag in both directions |
